...
void FDSelection::PandizzleAlg::FillTrackVariables(const art::Ptr<recob::PFParticle> pfp, const art::Event& evt){
  ...
  for (unsigned int i_pid = 0; i_pid < particle_ids.size(); i_pid++){
    ...
    std::vector<anab::sParticleIDAlgScores> particle_id = particle_ids.at(i_pid)->ParticleIDAlgScores();
     if (particle_ids.at(i_pid)->PlaneID().Plane != 2) continue;

    //art::Ptr<anab::ParticleID> particle_id = particle_ids[i_pid];
    for (size_t i_id = 0; i_id < particle_id.size(); i_id++)
    {
      anab::sParticleIDAlgScores AlgScore = particle_id.at(i_id);
      if(AlgScore.fAlgName== "PIDA"){
        fVarHolder.FloatVars["PFPTrackPIDA"] = std::min(AlgScore.fValue,float(50.));
        }
    }
